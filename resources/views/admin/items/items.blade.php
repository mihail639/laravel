  ...
  <div id="inner-content">
  	...
  	<section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1>{{ $pagesTitle['items'] }}</h1>
            </div>
            <div class="col-sm-6">
              <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="/admin">Главная</a></li>
                <li class="breadcrumb-item active">{{ $pagesTitle['items'] }}</li>
              </ol>
            </div>
          </div>
        </div><!-- /.container-fluid -->
          ...
          <div class="card">
              <div class="card-header">
                <h3 class="card-title">
                    ...
                    @else
                      @if($search == '')
                        Записей нет, <a onclick="addItem('form');" href="#">добавить запись</a>
                      @else
                  ...
                  <div class="row">
                    ...
                    <div class="col-sm-12 col-md-4">
                      <div id="example1_filter" class="text-center">
                        <button class="add-button action-buttons" onclick="addItem('form');">Добавить запись</button>
                      </div>
                      <div id="example1_filter" class="text-center">
                        <button id="delete-all-button" class="delete-all-button action-buttons" onclick="deleteChecked();" style="display:none;">Удалить отмеченные</button>
                      </div>
                    </div>
                    <div class="col-sm-12 col-md-4">
                    ...
                    <div class="col-sm-12">
                      <table id="example1" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example1_info">
                        ...
                        <tr role="row">
                          <th rowspan="1" colspan="1">
                            <div class="form-group mb-0">
                              <div class="custom-control custom-checkbox">
                                <input class="custom-control-input" type="checkbox" id="checkForDel" onchange="changeTableCheckbox(this,'checkForDel2')">
                                <label for="checkForDel" class="custom-control-label">Все</label>
                              </div>
                            </div>
                          </th>
                          ...
                          <th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: auto;">
                            Тизер
                          </th>
                  ...
                  <tbody>
                    @foreach($query as $item)
                      <tr role="row" class="odd table-item-{{ $item->id }}">
                        <td class="sorting_1">
                          <div class="form-group table-checkbox mb-0">
                              <div class="custom-control custom-checkbox">
                                <input class="custom-control-input" type="checkbox" id="checkForDel{{ $item->id }}" value="{{ $item->id }}" onchange="changeTableCheckbox(this,'','single')">
                                <label for="checkForDel{{ $item->id }}" class="custom-control-label"></label>
                              </div>
                            </div>
                        </td>
                        <td class="sorting_1">{{ $item->id }}</td>
                        <td class="sorting_1">{{ $item->title_ru }}</td>
                        <td>{{ $item->teaser_ru }}</td>
                        <td> @if($item->see == 1) Видно @else Не видно @endif </td>
                        <td>{{ $item->created_at }}</td>
                        <td>{{ $item->updated_at }}</td>
                        <td>
                          <button class="edit-button action-buttons" onclick="editItem('form',{{ $item->id }});"></button>
                          <button class="delete-button action-buttons" onclick="deleteItem({{ $item->id }});"></button>
                  ...
                  <tfoot>
                    <tr>
                      <th rowspan="1" colspan="1">
                        <div class="form-group mb-0">
                          <div class="custom-control custom-checkbox">
                            <input class="custom-control-input" type="checkbox" id="checkForDel2" onchange="changeTableCheckbox(this,'checkForDel')">
                            <label for="checkForDel2" class="custom-control-label">Все</label>
                          </div>
                        </div>
                      </th>  
                      <th rowspan="1" colspan="1">
